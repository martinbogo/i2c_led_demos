...
static vec3_t raise_drone_camera(vec3_t cam, float clearance) {
    float floor = terrain_height(cam.x, cam.z) + clearance;
...
static void draw_scene_voxel_plane(float scene_t, unsigned phase) {
    ...
    vec3_t cam, focus;
    float scene_scale;
    paper_plane_drone_camera(scene_t, plane, &cam, &focus, &scene_scale);
