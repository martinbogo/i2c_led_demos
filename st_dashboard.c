 ...
 *
 * st_dashboard.c - LCARS Star Trek Style System Dashboard
 * 
 * Features:
 * - 15-second dynamic scene rotation (Core Temp, Memory/Storage, Comm-Link, Sys Topo)
 * - Linux hardware telemetry parsers (sysinfo, statvfs, /proc/net/dev, thermal)
...
static void __attribute__((unused)) clear_rect(int x0, int y0, int x1, int y1) {
    if (x0 > x1) { int t = x0; x0 = x1; x1 = t; }
    if (y0 > y1) { int t = y0; y0 = y1; y1 = t; }
    for (int y = y0; y <= y1; y++)
        for (int x = x0; x <= x1; x++)
...
static void __attribute__((unused)) fill_circle(int cx, int cy, int r) {
    for (int y = -r; y <= r; y++)
        for (int x = -r; x <= r; x++)
...
static void __attribute__((unused)) draw_str_inv(int x, int y, const char *s) {
    while (*s) {
        char c = *s++;
        if (c < 32 || c > 126) c = '?';
        const uint8_t *g = font5x7[c - 32];
...
static void draw_alert_pips(int x, int y, unsigned int current, unsigned int recent) {
    ...
    for (int i = 0; i < 8; i++) {
        int px0 = x + i * 5;
        if (recent & bits[i])
            outline_rect(px0, y, px0 + 3, y + 3);
        if (current & bits[i])
            fill_rect(px0 + 1, y + 1, px0 + 2, y + 2);
    }
}
...
static unsigned int read_throttled_mask(void) {
    ...
    unsigned int mask = 0;
    if (!p) return 0;
    if (fgets(line, sizeof(line), p)) sscanf(line, "throttled=0x%x", &mask);
    pclose(p);
    return mask;
}
...
static int get_fan() {
    ...
    /* 1. Try hwmon interface for precise RPM (Common on Pi 5 Active Cooler) */
    for (int i = 0; i < 5; i++) {
        ...
        if (f) {
            if (fscanf(f, "%d", &val) == 1) {
                fclose(f);
                return val; /* Native RPM found */
            }
            fclose(f);
        }
    }
    
    /* 2. Try sysfs thermal tree as fallback */
    for (int i = 0; i < 5; i++) {
        ...
        if (f) {
            ...
            /* Verify the cooling device is actually a physical active fan */
...
static void get_disk_io(unsigned long long *rd, unsigned long long *wr) {
    *rd = 0; *wr = 0;
    FILE *f = fopen("/proc/diskstats", "r");
    if (f) {
        char line[256];
        while (fgets(line, sizeof(line), f)) {
            if (strstr(line, "mmcblk0 ") || strstr(line, "sda ") || strstr(line, "nvme0n1 ")) {
                unsigned long long r_sec, w_sec;
                if (sscanf(line, "%*d %*d %*s %*d %*d %llu %*d %*d %*d %llu", &r_sec, &w_sec) == 2) {
                    *rd += r_sec * 512;
                    *wr += w_sec * 512;
                }
            }
...
static void update_telemetry(float sim_t) {
    ...
    for (int i = 0; i < core_count; i++)
        g_telemetry.core_freq_mhz[i] = read_cpu_freq_mhz(i);
...
static void draw_lcars_frame(float sim_t, int scene, const char *title) {
    /* 1. Hollow Top Yellow Bar */
    fill_rect(0, 0, WIDTH - 12, 0);       // Top edge
    fill_rect(0, 14, WIDTH - 12, 14);     // Bottom edge
    fill_rect(0, 0, 0, 14);               // Left edge
    
    /* Smooth Elbow Outline */
    for(float a = -M_PI/2; a <= M_PI/2; a+=0.05f) {
        px(WIDTH - 12 + (int)(7*cosf(a)), 7 + (int)(7*sinf(a)));
    }
    
    /* 2. Subtle random title drift for burn-in protection */
    char buf[24];
    ...
    /* 3. Left-side LCARS stack with curved top panel */
    ...
    // Pulsing hardware heartbeat
    if ( ((int)(sim_t * 6.0f)) % 2 == 0 ) px(7, 31);
}

static void __attribute__((unused)) format_bytes(unsigned long long b, char *out) {
    if (b >= 1099511627776ULL) snprintf(out, 16, "%lluTB", b / 1099511627776ULL);
    else if (b >= 1073741824ULL) snprintf(out, 16, "%lluGB", b / 1073741824ULL);
    else if (b >= 1048576ULL) snprintf(out, 16, "%lluMB", b / 1048576ULL);
    else if (b >= 1024ULL) snprintf(out, 16, "%lluKB", b / 1024ULL);
    else snprintf(out, 16, "%lluB", b);
}
